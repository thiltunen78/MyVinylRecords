# My Vinyl Records
ESP8266 microcontroller project to track my vinyl record listening times. The project is coded using the Arduino IDE.
...
- When a record is selected from the list a new view is opened to show information from the record entry. For example listening count, last played date and some notes. In this view it is possible to increase listening count with one or two counts. When count is icreased the data is saved to the SD card.
